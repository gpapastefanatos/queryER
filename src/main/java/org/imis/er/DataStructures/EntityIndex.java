...
//            System.err.println("Entity out of limits!!!!");
            return null;
//            System.exit(-1);
        }
    ...
    private void indexBilateralEntities(List<AbstractBlock> blocks) {
        validEntities1 = 0;
        validEntities2 = 0;
        final List<Integer>[] entityIndexList = new ArrayList[noOfEntities];
        for (AbstractBlock block : blocks) {
            BilateralBlock bilBlock = (BilateralBlock) block;
            for (int id1 : bilBlock.getIndex1Entities()) {
                if (entityIndexList[id1] == null) {
                    validEntities1++;
                    entityIndexList[id1] = new ArrayList<Integer>();
                }
                entityIndexList[id1].add(block.getBlockIndex());
            }
            
            for (int id2 : bilBlock.getIndex2Entities()) {
                if (entityIndexList[datasetLimit+id2] == null) {
                    validEntities2++;
                    entityIndexList[datasetLimit+id2] = new ArrayList<Integer>();
                }
                entityIndexList[datasetLimit+id2].add(block.getBlockIndex());
            }
        }
        
        entityBlocks = new int[noOfEntities][];
        for (int id = 0; id < noOfEntities; id++) {
            if (entityIndexList[id] == null) {
                entityBlocks[id] = null;
                continue;
            }

            Collections.sort(entityIndexList[id]);
            entityBlocks[id] = Converter.convertListToArray(entityIndexList[id]);
        }
    }
    ...
    private void indexUnilateralEntities(List<AbstractBlock> blocks) {  
        ...
        for (AbstractBlock block : blocks) {
            ...
            for (int id : uniBlock.getEntities()) {
                if (entityIndexList[id] == null) {
                    validEntities1++;
                    entityIndexList[id] = new ArrayList<Integer>();
                }
                entityIndexList[id].add(block.getBlockIndex());
            }
        }
