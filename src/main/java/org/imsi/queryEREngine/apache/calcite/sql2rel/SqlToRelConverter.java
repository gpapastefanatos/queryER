...
import org.imsi.queryEREngine.apache.calcite.rel.hint.HintStrategyTable;
...
import org.imsi.queryEREngine.apache.calcite.rel.stream.Delta;
import org.imsi.queryEREngine.apache.calcite.rel.stream.LogicalDelta;
...
import org.imsi.queryEREngine.apache.calcite.rex.RexCallBinding;
...
import org.imsi.queryEREngine.apache.calcite.rex.RexPatternFieldRef;
import org.imsi.queryEREngine.apache.calcite.rex.RexRangeRef;
...
import org.imsi.queryEREngine.apache.calcite.sql.JoinConditionType;
...
import org.imsi.queryEREngine.apache.calcite.sql.SqlDelete;
import org.imsi.queryEREngine.apache.calcite.sql.SqlDynamicParam;
import org.imsi.queryEREngine.apache.calcite.sql.SqlExplainFormat;
...
import org.imsi.queryEREngine.apache.calcite.sql.SqlJoin;
import org.imsi.queryEREngine.apache.calcite.sql.SqlKind;
import org.imsi.queryEREngine.apache.calcite.sql.SqlLiteral;
import org.imsi.queryEREngine.apache.calcite.sql.SqlMatchRecognize;
import org.imsi.queryEREngine.apache.calcite.sql.SqlMerge;
import org.imsi.queryEREngine.apache.calcite.sql.SqlNode;
import org.imsi.queryEREngine.apache.calcite.sql.SqlNodeList;
import org.imsi.queryEREngine.apache.calcite.sql.SqlNumericLiteral;
...
import org.imsi.queryEREngine.apache.calcite.sql.SqlOrderBy;
import org.imsi.queryEREngine.apache.calcite.sql.SqlSampleSpec;
import org.imsi.queryEREngine.apache.calcite.sql.SqlSelect;
import org.imsi.queryEREngine.apache.calcite.sql.SqlSelectKeyword;
import org.imsi.queryEREngine.apache.calcite.sql.SqlSetOperator;
import org.imsi.queryEREngine.apache.calcite.sql.SqlSnapshot;
import org.imsi.queryEREngine.apache.calcite.sql.SqlUnnestOperator;
import org.imsi.queryEREngine.apache.calcite.sql.SqlUpdate;
import org.imsi.queryEREngine.apache.calcite.sql.SqlUtil;
import org.imsi.queryEREngine.apache.calcite.sql.SqlValuesOperator;
import org.imsi.queryEREngine.apache.calcite.sql.SqlWindow;
import org.imsi.queryEREngine.apache.calcite.sql.SqlWith;
import org.imsi.queryEREngine.apache.calcite.sql.SqlWithItem;
import org.imsi.queryEREngine.apache.calcite.sql.fun.SqlCase;
import org.imsi.queryEREngine.apache.calcite.sql.fun.SqlCountAggFunction;
import org.imsi.queryEREngine.apache.calcite.sql.fun.SqlInOperator;
import org.imsi.queryEREngine.apache.calcite.sql.fun.SqlQuantifyOperator;
import org.imsi.queryEREngine.apache.calcite.sql.fun.SqlRowOperator;
...
import org.imsi.queryEREngine.apache.calcite.sql.type.TableFunctionReturnTypeInference;
...
import org.imsi.queryEREngine.apache.calcite.sql.validate.CollectNamespace;
import org.imsi.queryEREngine.apache.calcite.sql.validate.DelegatingScope;
import org.imsi.queryEREngine.apache.calcite.sql.validate.ListScope;
import org.imsi.queryEREngine.apache.calcite.sql.validate.MatchRecognizeScope;
import org.imsi.queryEREngine.apache.calcite.sql.validate.ParameterScope;
...
import org.imsi.queryEREngine.apache.calcite.sql.validate.SqlNameMatcher;
import org.imsi.queryEREngine.apache.calcite.sql.validate.SqlQualified;
import org.imsi.queryEREngine.apache.calcite.sql.validate.SqlUserDefinedTableFunction;
import org.imsi.queryEREngine.apache.calcite.sql.validate.SqlUserDefinedTableMacro;
...
import org.imsi.queryEREngine.apache.calcite.sql.validate.SqlValidatorTable;
 ...
 * {@link org.imsi.queryEREngine.apache.calcite.sql.SqlNode} objects) into a relational algebra
 * expression (consisting of {@link org.imsi.queryEREngine.apache.calcite.rel.RelNode} objects).
 *
...
public class SqlToRelConverter {
	...
	public RelNode trimUnusedFields(boolean ordered, RelNode rootRel) {
		...
		if (isTrimUnusedFields()) {
			...
			rootRel = trimmer.trim(rootRel); //THIS ADDS THE PROJECT
	 ...
	 * {@link org.imsi.queryEREngine.apache.calcite.rel.logical.LogicalProject}s so that we are
	 ...
	 *
	 * @param bb               Blackboard
	 * @param ch eckForDupExprs Check for duplicate expressions
	 */
	 ...
	 * If deduplicate is true then above the Filter we add a MergeEntities.
	 * @param bb    Blackboard
	 ...
	 * ({@link org.imsi.queryEREngine.apache.calcite.rel.logical.LogicalValues}) rather than a
	 ...
	 *             </ul>
	 * @param deduplicate If true we either change the Join Type or pass it to the other cases
	 * 					  so we can put the Deduplicate rel op above the Scan
	 */
	protected void convertFrom(
		...
		case MATCH_RECOGNIZE:
			convertMatchRecognize(bb, (SqlCall) from, deduplicate);
			return;

		case AS:
			call = (SqlCall) from;
			convertFrom(bb, call.operand(0), deduplicate);
		...
		case WITH_ITEM:
			convertFrom(bb, ((SqlWithItem) from).query, deduplicate);
			return;

		case WITH:
			convertFrom(bb, ((SqlWith) from).body, deduplicate);
			return;
		...
		case TABLESAMPLE:
			...
			if (sampleSpec instanceof SqlSampleSpec.SqlSubstitutionSampleSpec) {
				...
				convertFrom(bb, operands[0], deduplicate);
				datasetStack.pop();
		...
		case TABLE_REF:
			call = (SqlCall) from;
			convertIdentifier(bb, call.operand(0), null, call.operand(1), deduplicate);
			return;

		case IDENTIFIER:
			convertIdentifier(bb, (SqlIdentifier) from, null, null, deduplicate);
			return;
		...
		case EXTEND:
			...
			final SqlIdentifier id = operand0.getKind() == SqlKind.TABLE_REF
					...
					convertIdentifier(bb, id, extendedColumns, null, deduplicate );
					return;

		case SNAPSHOT:
			convertTemporalTable(bb, (SqlCall) from, deduplicate);
			return;
		...
		case JOIN:
			...
			convertFrom(leftBlackboard, left, deduplicate);
			RelNode leftRel = leftBlackboard.root;
			convertFrom(rightBlackboard, right, deduplicate);
			RelNode rightRel = rightBlackboard.root;
			
			//If Deduplicate, choose the corresponding join type
			if(deduplicate) {
				joinType = JoinType.CLEAN;
			}
	...
	protected void convertMatchRecognize(Blackboard bb, SqlCall call, Boolean deduplicate) {
		...
		convertFrom(matchBb, expr, deduplicate);
	 ...
	 * @param tableHints
	 * @param deduplicate if true then above the Scan we add a Deduplicate
	 * 
	 * TODO: change the type of CsvTableScan to something more general
	 * 
	 */
	private void convertIdentifier(Blackboard bb, SqlIdentifier id,
			SqlNodeList extendedColumns, SqlNodeList tableHints, Boolean deduplicate) {
		final SqlValidatorNamespace fromNamespace =
				validator.getNamespace(id).resolve();
		if (fromNamespace.getNode() != null) {
			convertFrom(bb, fromNamespace.getNode(), false);
			return;
		}
		...
		Integer key = 0;
		Source source = null;
		List<CsvFieldType> fieldTypes = null;
		
		if(tableRel instanceof CsvTableScan) {
			key = ((CsvTableScan) tableRel).csvTable.getKey();
			source = ((CsvTableScan) tableRel).csvTable.getSource();
	...
	private void convertTemporalTable(Blackboard bb, SqlCall call, Boolean deduplicate) {
		...
		convertFrom(bb, expr, deduplicate);
	...
	protected RelNode createDeduplicateJoin(
			Blackboard bb,
			RelNode leftRel,
			RelNode rightRel,
			RexNode joinCond,
	...
	private static JoinRelType convertJoinType(JoinType joinType) {
		...
		case CLEAN:
			return JoinRelType.CLEAN;
		case DIRTY:
			return JoinRelType.DIRTY;
		default:
	...
	private void convertSelectList(
			Blackboard bb,
			SqlSelect select,
			List<SqlNode> orderList, boolean isDedup) {
		SqlNodeList selectList = select.getSelectList();
	...
	public interface Config {
		 ...
		 * of {@link org.imsi.queryEREngine.apache.calcite.rel.logical.LogicalValues} are generated.
		 ...
		 * {@link org.imsi.queryEREngine.apache.calcite.rex.RexSubQuery}. */
		boolean isExpand();
		 ...
		 * table ({@link org.imsi.queryEREngine.apache.calcite.rel.logical.LogicalValues}) rather than
