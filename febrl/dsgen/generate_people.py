...
# #all data, comment this if you want split
...
email_dict = {'name':'email',
                 'type':'freq',
           'char_range':'alpha',
            'freq_file':'data'+os.sep+'email-freq.csv',
          'select_prob':0.15,
             'ins_prob':0.10,
             'del_prob':0.15,
             'sub_prob':0.55,
...
ssid_key = {'name':'soc_sec_id',
		'type':'freq',
        'char_range':'alpha',
         'freq_file':'data'+os.sep+'key-freq.csv',
       'select_prob':0.00,
          'ins_prob':0.00,
          'del_prob':0.00,
          'sub_prob':0.00,
...
while (rec_cnt < num_org_records):
      ...
      rand_val = missing_value
    	...
    	if(is_join):
	    	if(field_name == join_key):
	      		if(rand_val not in join_keys):
	      			join_keys.add(rand_val)
		      		while(True):
  ...
  # modify e-mail here because before we don't know the values
  rec_dict["email"] = modify_email(rec_dict)
  ...
  if(not is_join):
	try:
		key_out = open("data/key-freq.csv" , 'w')
	except:
	    print 'Error: Can not write to key file data/key-freq.csv'
	    
	org_rec_ids = org_rec.keys()
	for org_rec_id in org_rec_ids:
	    key_out.write(str(org_rec.get(org_rec_id).get(join_key)) +",1" + "\n")
