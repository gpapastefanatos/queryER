...
funder_dict = {'name':'funder',
								 'type':'freq',
					 'char_range':'alpha',
						'freq_file':'data'+os.sep+'universities-freq.csv',
					'select_prob':0.10,
						 'ins_prob':0.04,
						 'del_prob':0.04,
						 'sub_prob':0.10,
...
projectCode_dict = {'name':'projectCode',
								 'type':'ident',
						'start_id': 10000,
						'end_id': 99999999,		 
					 'char_range':'digit',
...
fundingLevel0_dict = {'name':'fundingLevel0',
								 'type':'freq',
					 'char_range':'alpha',
						'freq_file':'data'+os.sep+'universities-freq.csv',
					'select_prob':0.05,
...
fundingLevel1_dict = {'name':'fundingLevel1',
								 'type':'freq',
					 'char_range':'alpha',
						'freq_file':'data'+os.sep+'universities-freq.csv',
					'select_prob':0.05,
...
organisation_dict = {'name':'organisation',
								 'type':'freq',
					 'char_range':'alpha',
						'freq_file':'data'+os.sep+'universities-freq.csv',
					'select_prob':0.10,
...
field_list = [title_dict, startDate_dict, endDate_dict,	funder_dict,	projectCode_dict,	fundingLevel0_dict,	fundingLevel1_dict, organisation_dict]			
# -----------------------------------------------------------------------------
...
def error_character(input_char, char_range):
	...
	rows = {'a':'s',	'b':'vn', 'c':'xv', 'd':'sf', 'e':'wr', 'f':'dg', 'g':'fh',
					...
					'A':'S',	'B':'VN', 'C':'XV', 'D':'SF', 'E':'WR', 'F':'DG', 'G':'FH',
					'H':'GJ', 'I':'UO', 'J':'HK', 'K':'JL', 'L':'K',	'M':'N',	'N':'BM',
					'O':'IP', 'P':'O',	'Q':'W',	'R':'ET', 'S':'AD', 'T':'RY', 'U':'YI',
					'V':'CB', 'W':'QE', 'X':'ZC', 'Y':'TU', 'Z':'X', 
	...
	cols = {'a':'qzw','b':'gh', 'c':'df', 'd':'erc','e':'d', 'f':'rvc','g':'tbv',
					...
					'A':'QZW','B':'GH', 'C':'DF', 'D':'ERC','E':'D', 'F':'RVC','G':'TBV',
					'H':'YBN','I':'K',	'J':'UMN','K':'IM', 'L':'O',	'M':'JK', 'N':'HJ',
					'O':'L',	'P':'P',	'Q':'A',	'R':'F',	'S':'WXZ','T':'GF',	'U':'J',
					'V':'FG', 'W':'S',	'X':'SD', 'Y':'H',	'Z':'AS'}
...
def modify_email(rec_dict):

	try:
		name = rec_dict["given_name"]
		surname = rec_dict["surname"]
		email = rec_dict["email"]

		if(name != None):
			rand_trans_pos = error_position(name, -1)

			trans_chars = name[rand_trans_pos:rand_trans_pos+2]
			trans_chars2 = trans_chars[1] + trans_chars[0]	# Do transposition

			name = name[:rand_trans_pos] + trans_chars2 + \
												name[rand_trans_pos+2:]
		else:
			return ""

		if(surname != None):
			rand_trans_pos = error_position(surname, -1)

			trans_chars = surname[rand_trans_pos:rand_trans_pos+2]
			trans_chars2 = trans_chars[1] + trans_chars[0]	# Do transposition

			surname = surname[:rand_trans_pos] + trans_chars2 + \
												surname[rand_trans_pos+2:]
		else:
			return ""
...
import pandas
colnames = ['name']
data = pandas.read_csv('data/distinct_orgs.csv', names=colnames)

organisations = data.name.tolist()
		...
		for i, x in enumerate(lis):							#print the list items 
			...
			for j, col in enumerate(x):
				...
				if (i == 0):
					...
					rec_dict["rec_id"] = str(i)
					rec_dict[attrs[j]] = col
			rec_dict["organisation"] = random.choice(organisations)

			org_rec[str(i)] = rec_dict
...
ground_truth = {} # Dictionary for ground truth linkset

rec_cnt = 0	# Record counter

num_org_records = len(org_rec)
print(num_org_records)
while (rec_cnt < num_dup_records):
	...
	while (d < num_dups) and (rec_cnt < num_dup_records):
		...
		dup_rec_id = str(rand_rec_num + d + num_org_records)
		dup_rec_dict['rec_id'] = dup_rec_id
		ground_truth[rand_rec_num + d + num_org_records] = rand_rec_num
...
#
try:
	gt_out = open("ground_truth_" + output_file, 'w')
except:
	print 'Error: Can not write to ground truth file "%s"' % ("ground_truth" + output_file)

gt_out.write('id_d, id_s\n')
dup_rec_ids = ground_truth.keys()

for dup_rec_id in dup_rec_ids:
	gt_out.write(str(dup_rec_id) + "," + str(ground_truth.get(dup_rec_id)) + "\n")
print 'End.'
